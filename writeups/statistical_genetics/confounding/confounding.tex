...
\section{General Generative Model for a Phenotype}
...
\begin{itemize}
    ...
    \item \textbf{Structural Variants (SVs)}: [To-do]
    \item \textbf{Copy-Number Variants (CNVs)}: [To-do]
    \item ... [To-do]
\end{itemize}
...
As stated, we assume this causal effect is both linear and independent of other effects (such that is additive).
...
This exercise reveals that there are two types of confounding, discussed in the next two sections.
...
The recombination fraction between two variants, $c_{jk}$, quantifies the probability of an odd number of crossover events occurring between the two loci, which causes two alleles that are in cis to segregate to different gametes.
...
In practice, the field assumes a fuzzy recombination fraction threeshold for which two variants below the threshold are considered sufficiently linked and are said to be \textbf{tagging} or \textbf{local} to each other.
