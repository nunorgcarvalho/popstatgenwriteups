...
$ \Corr[\,g_m, g_p\,] = \rhogt $, where the superscript records that --- unlike $\rho_y$, which is held fixed --- this induced correlation changes from one generation to the next.
...
That superscript is doing real work, so we adopt it as a convention: a parenthesized superscript names the generation a quantity belongs to, $q^{(t)}$, with $q^{(0)}$ the randomly mating base population and $q^{(\mathrm{eq})}$ the equilibrium reached under sustained assortment.
Assortative mating inflates $\Var[y]$ from one generation to the next, so $\VPt$ moves and $\mut$ moves with it; the same holds for $\VAt$, $\hsqt$, and the genetic correlation between mates $\rhogt$.
Of the quantities in this section only $\rho_y$ and $V_E$ are generation-invariant, and they are the only two we write without a superscript.
