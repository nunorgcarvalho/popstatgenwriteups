...
We assume a model of assortative mating based on mates' phenotypic similarity.

\begin{assumption}{Primary-AM}{Primary Assortative Mating}
Mating is non-random such that two mates' phenotypes are correlated for some trait:
$$ \Corr[\,y_m, y_p\,] = \rho_y , $$
where the subscripts $m$ and $p$ index the maternal and paternal member of a mated pair, matching the allele-origin superscripts of \eqref{eq:Adecomp}.
...
The distinction that \assumptionid{Primary-AM} draws --- matching on the phenotype, everything else derived --- is easy to state and easy to get wrong when computing with it.
The temptation is to write down the induced component covariances directly, one per pair of components; but they are not free parameters, and imposing them by hand both over-determines the model and, as we will see, gets the wrong answer for anything upstream of the phenotype.
The next section introduces the machinery that lets us specify only the phenotypic matching and \emph{read off} everything it implies.

\subsection{Dynamics and equilibrium}
When starting from a randomly mating population, one generation of assortative mating causes the immediate offspring generation to exhibit different phenotypic and genetic properties.
...
First, we'll introduce path analysis, a very helpful tool for modelling the covariance between variables in a causal system, particularly under assortative mating.

A \emph{path diagram} is a drawing of a generative model that is complete enough to compute with.
Each variable is a node --- drawn as a rectangle if it is observed and an ellipse if it is latent --- and the nodes are joined by three kinds of edge:
...
\begin{itemize}
    ...
    \item A \textbf{bidirected} edge $a \leftrightarrow b$ carries covariance between $a$ and $b$ that the diagram does not otherwise account for.
    The self-loop $a \leftrightarrow a$ is the case $b=a$: it is $a$'s \emph{exogenous} variance, the part not supplied by the arrows pointing into it.
    A variable with no incoming arrows carries its entire variance on such a loop.
    ...
    This third edge type is due to \citet{Sunde2025NatComm} and is what lets us encode assortative mating without also asserting things we should be deriving.
\end{itemize}

The reason to draw the diagram rather than simply write the equations is that the covariance between \emph{any} two variables in it --- latent and intermediate ones included --- can be read straight off the picture, by Wright's tracing rules.
...
Note that a variable may legitimately appear on both legs of a chain: the classical rule forbidding this belongs to the standardized formulation, in which a chain may turn around at any variable using $\Var=1$ implicitly.
Here a turning point must be written out as an explicit $u \leftrightarrow u$, and for a variable with parents that loop carries only its \emph{residual} variance --- the rest arrives on chains that continue back to its ancestors and therefore pass through it twice.
...
It \emph{may} cross co-paths of different mating processes, and it must be able to --- that is how the effect of assortment accumulates down a pedigree.

This is precisely where a co-path differs from a bidirected edge, and the difference is not cosmetic.
A bidirected edge is a statement about \emph{disturbances}, so a chain terminates on it: it cannot reach past $y_m$ to the things that caused $y_m$.
...
Making that concrete: if the co-path in Figure~\ref{fig:pair-2v} is replaced by a bidirected edge $y_m \leftrightarrow y_p$ of the same magnitude, every cross-mate entry involving a genotype or a genetic value collapses to exactly zero, and only the two phenotypes remain correlated.
So the rule to work by is that \textbf{the covariance between mates' genetic components is derived from the phenotypic matching, never specified alongside it} --- which is exactly what \assumptionid{Primary-AM} asserts, now in a form we can compute with.

One detail matters for getting the numbers right.
...
\begin{equation}\label{eq:copath-mu}
    \mu \;=\; \frac{\rho_y}{\sqrt{\Var[y_m]\,\Var[y_p]}} \;=\; \frac{\rho_y}{\VPo} ,
\end{equation}
because a co-path contributes $\Cov[a,b] = \mu\,\Var[a]\,\Var[b]$ \citep{Sunde2025NatComm}.
The two coincide only when the matched variables have unit variance.
Since assortative mating inflates $\Var[y]$ from one generation to the next, $\mu$ is generation-indexed even though $\rho_y$ is held fixed, and a value of $\mu$ carried from one generation to the next is silently wrong.
For this reason we label a co-path with its correlation in brackets, as $[\rho_y]$, and leave $\mu$ to be derived from the diagram's own variances.
...
The chain enumeration is the one worth having here, because it returns the derivation and not only the answer.
...
The parameters defined in the previous section, such as the additive variance $V_A$ and heritability $h^2$, are measured in this base population and denoted by a $(0)$ superscript, e.g. $\VAo$ and $\hsqo$, since these terms do not remain fixed across generations under assortative mating.
Note that $V_E$ remains fixed.

Then, mates in generation $t=0$ are formed according to \assumptionid{Primary-AM}, the offspring generation $t=1$ will have a different distribution of phenotypic and genetic values than the base population.
...
\begin{figure}[htbp]
    ...
    \caption{%
        ...
        Unit coefficients are usually left off a path diagram; they are shown here so that every factor a reader multiplies along a chain is visible.
        \emph{Everything} else about the assortment --- every entry in Table~\ref{tab:pair-2v-between} --- is derived from this one edge.%
    }
    \label{fig:pair-2v}
\end{figure}

Note what the diagram does \emph{not} contain.
There is no edge between $g_m$ and $g_p$, none between any two genotypes, and none between one partner's environment and the other's genetic value.
The only thing asserted about the pair is that their phenotypes are matched, which is all \assumptionid{Primary-AM} claims; everything else is a consequence to be traced out.

Figure~\ref{fig:pair-2v-traced} works one such consequence through in full.
It is the covariance between two \emph{different} causal variants in two \emph{different} people --- a quantity that is zero under random mating, and for which there is no common ancestor and no shared environment to appeal to.
A single chain connects them, and it is forced to pass through the co-path:
$$
    x_{m,1} \leftrightarrow x_{m,1} \rightarrow g_m \rightarrow y_m \;\text{---}\; y_p \leftarrow g_p \leftarrow x_{p,2} \leftrightarrow x_{p,2} ,
$$
whose factors multiply to $1 \cdot \beta_1 \cdot 1 \cdot \mu \cdot 1 \cdot \beta_2 \cdot 1$, giving $\Cov[x_{m,1}, x_{p,2}] = \beta_1\beta_2\rho_y/\VPo$ by \eqref{eq:copath-mu}.

\begin{figure}[htbp]
    \centering
    \input{figures/mated_pair_2v_traced.tikz}
    \caption{%
        The same model with one chain traced on it, and the product of its coefficients written underneath.
        ...
        The chain reaches backward through the co-path all the way to an individual variant: this is the property a bidirected edge does not have, and replacing the co-path with one would send this covariance to zero.%
    }
...
\begin{table}[htbp]
    ...
    \caption{%
        ...
        The last column --- each variable's covariance with its own phenotype --- is the only part of this table the next one needs.%
    }
    \label{tab:pair-2v-within}
\end{table}

Between the two mates the picture is entirely new, and it has a simple shape.
Every chain leaving one individual has to cross the co-path to reach the other, so every chain factors into a leg on the maternal side, the co-path coefficient, and a leg on the paternal side:
\begin{equation}\label{eq:leg-rule}
    \boxed{\ \Cov[\,a_m,\, b_p\,] \;=\; \Cov[\,a_m,\, y_m\,] \cdot \mu \cdot \Cov[\,y_p,\, b_p\,] \ }
\end{equation}
for any variables $a$ and $b$ belonging to the maternal and paternal member respectively.
The entire cross-mate block is therefore the outer product of a single vector --- the last column of Table~\ref{tab:pair-2v-within} --- scaled by $\mu = \rho_y/\VPo$.

\begin{table}[htbp]
    \centering
    \begin{tabular}{l|ccccc}
        $\times \; \rho_y/\VPo$ & $x_{p,1}$         & $x_{p,2}$         & $g_p$            & $e_p$          & $y_p$            \\
        \hline
    ...
    \end{tabular}
    \caption{%
        Covariances between the two mates. \textbf{Every entry carries a factor $\rho_y/\VPo$}, shown once in the corner rather than repeated in twenty-five cells.
...
Three entries of Table~\ref{tab:pair-2v-between} are worth naming, because each is an assumption from Section~1 breaking.

\begin{itemize}
    \item $\Cov[\,x_{m,1},\, x_{p,2}\,] = \beta_1\beta_2\,\rho_y/\VPo \neq 0$.
    ...
    One partner's environment is correlated with the other's genetic value, which is the break of \assumptionid{Env-indep} and \assumptionid{No-IGE} flagged at the start of this section.
    Nothing causal has been added --- no partner's genotype acts on the other's environment --- but the two are nonetheless associated, because each is a cause of a matched phenotype.
    \item $\Cov[\,g_m,\, g_p\,] = (\VAo)^2\rho_y/\VPo$, so that
    $$ \rho_g \;=\; \Corr[\,g_m,\, g_p\,] \;=\; \frac{(\VAo)^2 \rho_y/\VPo}{\VAo} \;=\; \rho_y\,\frac{\VAo}{\VPo} \;=\; \rho_y\,\hsqo . $$
    Partners' genetic values are correlated, but by less than their phenotypes are: the phenotypic correlation is attenuated by the heritability, since only the heritable part of the matched phenotype reaches the genotype.
    This is the base-population instance of the relation $\rho_g = \rho_y h^2$ derived below, and it is the quantity that drives everything that follows.
\end{itemize}
