...
The boxed line is that root with the numerator's difference of square roots cleared, which cancels one power of $\beta_k$ and leaves the $\beta_k^2 \to 0$ behaviour visible.
Setting $\rho_y = 0$ makes both $\mueq$ and $\rhogeq$ zero and returns $c^{(\mathrm{eq})}_k = \beta_k/2$, the base-population value of \eqref{eq:as-gen0}, as it must under random mating.
Note also that $\beta_k$ enters the denominator only as $\beta_k^2$, so a variant's sign is irrelevant to the size of its equilibrium covariance; $c^{(\mathrm{eq})}_k$ simply inherits it from the numerator.

However, we still have the problem that $\mueq$ and $\rhogeq$ depend on $\VAeq$, which itself depends on the sum of $c^{(eq)}_k$, the quantity we are solving for.
Summing \eqref{eq:c-eq} over variants does not resolve this, because each variant carries its own square root containing its own $2\mueq\beta_k^2$, so the $M$ terms cannot be gathered into one.
...
\begin{assumption}{Small-effects}{No variant explains an appreciable share of the additive variance}
Each causal variant's squared effect is a negligible fraction of the base population's additive variance:
$$\frac{\beta_k^2}{\VAo} \;\ll\; 1 \qquad \text{for every } k .$$
\bundles{the polygenic, or infinitesimal, model with large $M$.}
\end{assumption}
...
Because $\rhogeq = \rho_y\hsqeq$, reaching the ceiling requires $\rho_y$ and $\hsqeq$ to \emph{both} approach one, so this is a weak requirement in practice.
...
$$
    ...
    + \sqrt{\big(1-\rhogeq\big)^2 + \underbrace{2\,\mueq\,\beta_k^2}_{\text{negligible}}}}
    \;\approx\; \frac{\beta_k}{2\big(1-\rhogeq\big)}
$$
...
\;=\; \underbrace{\frac{2\,\rhogeq}{1-\rhogeq}}_{\text{\assumptionid{Moderate-AM}}} \cdot \underbrace{\frac{\beta_k^2}{\VAo}}_{\text{\assumptionid{Small-effects}}} ,$$
using $\mueq = \rhogeq/\VAeq$ together with the $\VAeq = \VAo/(1-\rhogeq)$ that this approximation itself yields below in \eqref{eq:VA-eq}.
The first factor is bounded by \assumptionid{Moderate-AM} and the second is small by \assumptionid{Small-effects}, so neither alone suffices.
All that's required is that the product of the two is close to 0.
Summed over variants, the resulting relative error in $\VAeq$ is of order $\dfrac{\rhogeq}{2\big(1-\rhogeq\big)} \cdot \dfrac{\sum_k\beta_k^4}{\big(\VAo\big)^2}$, in which the last factor is the reciprocal of the \emph{effective} number of variants and equals $1/M$ when effects are equal.}
\end{approximation}

The denominator no longer depends on $k$, so it comes straight out of the sum in \eqref{eq:VA-from-as}:
\begin{equation}\label{eq:VA-eq}
\begin{gathered}
    \VAeq \;=\; 2\sum_k \beta_k\, c^{(\mathrm{eq})}_k
    \;\approx\; 2\sum_k \beta_k \cdot \frac{\beta_k}{2\big(1-\rhogeq\big)}
    \;=\; \frac{\sum_k \beta_k^2}{1-\rhogeq} \\[6pt]
    \boxed{\ \VAeq \;=\; \frac{\VAo}{1-\rhogeq}\ }
\end{gathered}
\end{equation}
This formula for $\VAeq$ is easily interpretable, but $\rhogeq$ still relies on $\VAeq$ through \eqref{eq:rhog-t}.
...
\begin{aligned}
    \rhogeq \;&=\; \frac{\rho_y\,\VAeq}{\VAeq + V_E}
    \;=\; \rho_y\,\frac{\VAo/\big(1-\rhogeq\big)}{\VAo/\big(1-\rhogeq\big) + V_E} \\[4pt]
    \;&=\; \rho_y\,\frac{\VAo}{\VAo + \big(1-\rhogeq\big)V_E} \\[4pt]
    \rhogeq\,\VAo \;+\; \rhogeq\big(1-\rhogeq\big)V_E \;&=\; \rho_y\,\VAo \\[4pt]
    \big(\rhogeq\big)^2 V_E \;-\; \rhogeq\,\VPo \;+\; \rho_y\,\VAo \;&=\; 0 \\[4pt]
