...
$$
one $\tfrac12$ for each allele's share of its genotype, and one effect size for each of the \emph{two} variants involved --- not the square of either.
There are four such allele pairs behind any one pair of genotypes, and they are identical, so they sum back to $\Cov[x_{m,1}, x_{p,2}] = \beta_1\beta_2\rho_y/\VPo$.
...
$$
    \Var\!\big[z^{(u)}_{i,k}\big] = \tfrac12, \qquad
    \Cov\!\big[z^{(u)}_{i,k},\, z^{(v)}_{i,l}\big] = 0 \ \ \text{for } (u,k) \neq (v,l), \qquad
    \Cov\!\big[z^{(u)}_{i,k},\, y_i\big] = \tfrac{\beta_k}{2},
$$
for $u,v \in \{m,p\}$ naming the parental origin.
The middle one is two statements at once: an individual's own two alleles at the same variant are uncorrelated ($k = l$, the quantity named $\alpha$ below), and there is no gametic phase disequilibrium between different variants ($k \neq l$).
...
\begin{equation}\label{eq:seg-def}
    s^{(m)}_{ok} \;:=\; z^{(m)}_{ok} - \tfrac12 x_{mk} \;=\; \big(B^{(m)}_{ok} - \tfrac12\big)\big(z^{(m)}_{mk} - z^{(p)}_{mk}\big) ,
\end{equation}
which is mean-zero and uncorrelated with both of the mother's alleles.
Squaring \eqref{eq:seg-def} and using $\Var\big[B^{(m)}_{ok}\big] = \tfrac14$ gives its variance:
\begin{equation}\label{eq:seg-var}
    \Var\!\big[s^{(m)}_{ok}\big]
    \;=\; \tfrac14\,\E\!\big[(z^{(m)}_{mk} - z^{(p)}_{mk})^2\big]
    \;=\; \tfrac14\Big(\tfrac12 + \tfrac12 - 2\,\Cov\!\big[z^{(m)}_{mk}, z^{(p)}_{mk}\big]\Big)
    \;=\; \tfrac14 - \tfrac12\,\Cov\!\big[z^{(m)}_{mk}, z^{(p)}_{mk}\big] .
\end{equation}
The offspring's paternally inherited allele $z^{(p)}_{ok}$ is built the same way from the father's two alleles, with its own coin $B^{(p)}_{ok}$ and its own residual $s^{(p)}_{ok}$.
Every coin is independent of every other, across parental origins and across offspring: a second child of the same pair draws its own $B^{(m)}_{o'k}$ and $B^{(p)}_{o'k}$, which is what makes two siblings differ.
Following \citet{Yengo2018NatHumBehav} we define $\alpha^{(t)}_k$ as the \emph{correlation} of the maternally and paternally inherited alleles of an individual $i$ of generation $t$ at variant $k$:
\begin{equation}\label{eq:alpha-def}
    \alpha^{(t)}_k \;=\; \Corr\!\big[z^{(m)}_{ik},\, z^{(p)}_{ik}\big]
    \quad\Longrightarrow\quad
    \Cov\!\big[z^{(m)}_{ik},\, z^{(p)}_{ik}\big]
    \;=\; \tfrac12 \alpha^{(t)}_k .
\end{equation}

Of particular importance is the variance of the diploid genotype, which is inflated by $\alpha^{(t)}_k$:
\begin{equation}\label{eq:varx-alpha}
\begin{gathered}
    \begin{aligned}
        \Var[x_{ik}]
        &\;=\; \Var\!\big[z^{(m)}_{ik} + z^{(p)}_{ik}\big] \\
        &\;=\; \Var\!\big[z^{(m)}_{ik}\big] + \Var\!\big[z^{(p)}_{ik}\big] + 2\,\Cov\!\big[z^{(m)}_{ik}, z^{(p)}_{ik}\big] \\
        &\;=\; \tfrac12 + \tfrac12 + 2\cdot\tfrac{\alpha^{(t)}_k}{2}
    \end{aligned} \\[8pt]
    \boxed{\ \Var[x_{ik}] \;=\; 1 + \alpha^{(t)}_k \ }
\end{gathered}
\end{equation}
In the base population there is no inbreeding and mating is random, so $\alpha^{(0)}_k = 0$ for every variant, and everything reduces to the familiar $\Var[x_{ik}] = 1$.

Substituting \eqref{eq:alpha-def} back into \eqref{eq:seg-var} puts the segregation variance in the same terms:
\begin{equation}\label{eq:seg-var-alpha}
    \boxed{\ \Var\!\big[s^{(m)}_{ok}\big] \;=\; \frac{1 - \alpha^{(t)}_k}{4} \ }
\end{equation}
with $\alpha^{(t)}_k$ that of the transmitting parent.
A parent whose two alleles are more alike contributes \emph{less} segregation variance, because it matters less which of the two the offspring received; and in the base population, where $\alpha^{(0)}_k = 0$, it takes the familiar value $\tfrac14$.

Below is a path diagram of two offspring of a mated pair that assortatively mated on the focal trait with strength $\rho_y$, for a trait with two causal variants.
It is the diagram of Figure~\ref{fig:pair-2v} with transmission added and nothing else; every allele covariance in the offspring generation is traced out of it rather than written into it.
\begin{sidewaysfigure}
    \begin{floatcard}
        ...
        \caption{%
            ...
            The parents are drawn \emph{flowing upward} --- alleles at the bottom of each block, phenotype at the top --- so that the two generations' allele rows sit adjacent and the sixteen transmission edges stay short; the cost is that each offspring's $z \rightarrow x$ edges span the figure.
            Each inherited allele is fed by \emph{both} alleles of the parent it came from, at $\tfrac12$ each, per \eqref{eq:transmit-mean}.
            The self-loop of $\tfrac14$ on each offspring allele is its segregation variance, \eqref{eq:seg-var-alpha} evaluated at $\alpha^{(0)}_k = 0$: it is drawn as a disturbance variance because that is exactly what it is --- the part of the allele's variance its causes in the diagram do not supply.
            The two offspring differ only through their coins being independent; nothing else distinguishes them, and $e_{o1}$ and $e_{o2}$ are separate draws.
            Note that no $x$ anywhere carries a self-loop: $\Var[x] = 1 + \alpha^{(t)}_k$ is derived at both generations, and it takes the value $1$ in the parents and a larger one in the offspring without a single edge differing between them.%
        }
        \label{fig:pair-offspring-2v}
    \end{floatcard}
\end{sidewaysfigure}

We can use our path tracing rules to compute the correlation between maternally- and paternally-inherited alleles in the first offspring generation, $\alpha^{(1)}_k$:

Exactly four chains connect the offspring's two alleles at variant $k$.
Each traces back from $z^{(m)}_{ok}$ to \emph{one} of the mother's two alleles, turns around on that allele's variance, runs up through her genotype and phenotype, crosses the co-path, and comes back down the father's side to \emph{one} of his two alleles and into $z^{(p)}_{ok}$.
Two choices on each side give $2\times2 = 4$ chains, one for each $u, v \in \{m,p\}$:
$$
\begin{aligned}
    &z^{(m)}_{ok} \leftarrow z^{(u)}_{mk} \leftrightarrow z^{(u)}_{mk} \rightarrow x_{mk} \rightarrow g_m \rightarrow y_m \\
    \;\text{---}\;\;
    &y_p \leftarrow g_p \leftarrow x_{pk} \leftarrow z^{(v)}_{pk} \leftrightarrow z^{(v)}_{pk} \rightarrow z^{(p)}_{ok} ,
\end{aligned}
$$
broken across two lines at the co-path, which is exactly where it joins two valid chains into one.
They are identical in value, since every allele carries the same variance $\tfrac12$ and the same transmission coefficient $\tfrac12$:
$$
    \underbrace{\tfrac12}_{\text{transmission}} \cdot \underbrace{\tfrac12}_{\Var[z]} \cdot \beta_k \cdot \muo \cdot \beta_k \cdot \underbrace{\tfrac12}_{\Var[z]} \cdot \underbrace{\tfrac12}_{\text{transmission}}
    \;=\; \frac{\beta_k^2\,\muo}{16} .
$$
Summing the four and dividing by $\sqrt{\tfrac12\cdot\tfrac12} = \tfrac12$ to convert the covariance to a correlation,
\begin{equation}\label{eq:alpha-gen1}
    \alpha^{(1)}_k
    \;=\; \frac{4 \cdot \beta_k^2 \muo / 16}{1/2}
    \;=\; \frac{\beta_k^2\,\muo}{2}
    \;=\; \boxed{\ \frac{\beta_k^2\,\rho_y}{2\,\VPo} \ }
\end{equation}

We use the path tracing rules to compute the covariance between an individual and each of its two kinds of first-degree relative, at all three levels of the model.
The results are collected in Table~\ref{tab:relatives-gen1}; they are stated for $M$ variants, having been derived on the two-variant diagram.
Every entry there turns out to be its randomly mating value plus a single quantity, so we name that quantity rather than write it out five times:
\begin{equation}\label{eq:am-increment}
    \Delta^{(t)} \;:=\; \frac{\rho_y\,\big(V_A^{(t-1)}\big)^2}{2\,V_P^{(t-1)}}
    \;=\; \frac{V_A^{(t-1)}}{2}\,\rho_g^{(t-1)} ,
\end{equation}
the covariance that assortment adds at the genetic-value level among individuals of generation $t$.
It carries a generation index for the same reason $\alpha^{(t)}_k$ does, and follows the same convention: both are named for the generation they \emph{describe} while being computed from the variances of the parents who produced it, one generation back.
For the first offspring generation, $\Delta^{(1)} = \rho_y(\VAo)^2/(2\VPo)$.
Written this way nothing is hidden inside $\rho_g$: the increment is built only from $\rho_y$ and the variances $\VAo$ and $\VPo$.
...
\begin{table}[H]
    ...
    \begin{tabular}{l|ccc}
            ...
            & parent--offspring \\
        \hline
        genotypes, same variant $k$
            & $1 + \alpha^{(1)}_k$
            & $\tfrac12 + \alpha^{(1)}_k$
            & $\tfrac12 + \alpha^{(1)}_k$ \\
        genotypes, variants $k \neq l$
            & $\dfrac{\beta_k\beta_l\,\rho_y}{2\,\VPo}$
            & $\dfrac{\beta_k\beta_l\,\rho_y}{2\,\VPo}$
            & $\dfrac{\beta_k\beta_l\,\rho_y}{2\,\VPo}$ \\
        genetic values
            & $\VAo + \Delta^{(1)}$
        ...
        phenotypes
            & $\VAo + \Delta^{(1)} + V_E$
            & $\tfrac{\VAo}{2} + \Delta^{(1)}$
            & $\tfrac{\VAo}{2} + \dfrac{\rho_y \VAo}{2}$ \\
    \end{tabular}
    \caption{%
        Variances and covariances for an individual of the first generation produced under assortative mating, against itself and against each kind of first-degree relative: the ``self'' column is the pair $(o_1, o_1)$, ``full siblings'' is $(o_1, o_2)$, and ``parent--offspring'' is $(m, o_1)$.
        Every entry is written as its randomly mating value plus what assortment added, and each added term is built only from $\rho_y$ and the base-population variances: $\Delta^{(1)} = \rho_y(\VAo)^2/(2\VPo)$ from \eqref{eq:am-increment}, and $\alpha^{(1)}_k = \beta_k^2\rho_y/(2\VPo)$ from \eqref{eq:alpha-gen1}.
        Setting $\rho_y = 0$ kills both and returns the table to $1$ or $\tfrac12$, $0$, and $\VAo$ or $\VAo/2$.
        \textbf{The added term is the same in every cell but one}: the bottom right, where it is $\rho_y\VAo/2 = \Delta^{(1)}/\hsqo$, larger by a factor $1/\hsqo$.%
    }
    \label{tab:relatives-gen1}
    \end{floatcard}
\end{table}

Note that covariances for parents vs full-siblings only differ at the phenotypic level, and that the increment $\Delta^{(1)}$ is the same for both.
The entire difference is one extra term available to the lineal pair and not to the collateral one:
$$
    \Cov[\,y_m, y_{o_1}\,] \;=\; \underbrace{\Cov[\,g_m, g_{o_1}\,]}_{\frac{\VAo}{2}(1+\rhogo)} \;+\; \underbrace{\Cov[\,e_m, g_{o_1}\,]}_{\frac{\VAo V_E\,\rho_y}{2\,\VPo}} \;=\; \frac{\VAo}{2}\big(1+\rho_y\big) ,
$$
the second term being half the $\Cov[e_m, g_p] \neq 0$ we found between the mates themselves, inherited by their child.
A parent's \emph{environment} is correlated with their child's genetic value; two siblings' environments are not correlated with each other's.
This asymmetry between lineal and collateral relatives of the same degree is absent under random mating, and it is a signature of assortative mating rather than an artefact of it.
