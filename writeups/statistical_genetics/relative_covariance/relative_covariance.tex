...
\end{aligned} \\[8pt]
    \boxed{\ \Var[\varepsilon_o] \;=\; \tfrac12 V_A\big(1 - \rho_g\big) \ }
\end{gathered}
\end{equation}
The boxed form is \eqref{eq:VA-eq-exact} rearranged, since $V_A\big(1-\rho_g\big) = \VAo - 2\sum_k \beta_k^2 a_{kk}$.
Under random mating it reduces to the familiar $\tfrac12\VAo$, and each offspring's residual is the within-family variance: the spread in genetic value among children of the same two parents.
...
In a pedigree, the path between two individuals can be traced through a combination of three types of \emph{steps}: going up a meiosis (offspring $\rightarrow$ parent), going down a meiosis (parent $\rightarrow$ offspring), and crossing a mating (mother --- father).
To come up with a generalized formula for the covariance between any two such relatives later, it will help to split the shortest \emph{path} connecting the two into individual \emph{chains} that are separated by \emph{matings}.
For example, a path connecting individual A to individual D is better understood if split into a chain connecting A to B, then a mating connecting B to C, and then another chain connecting C to D.
Chains cannot consist of matings, and require the two individuals at the ends of the chain to have a common ancestor.
Note that this common ancestor can be one of the individuals at the chain's ends themselves, or even both individuals in the case where a chain simply connects an individual back to themselves (e.g.\ A $\rightarrow$ A).
Furthermore, for a particular chain, one needs to specify whether the two chain ends' individuals are connected by one common ancestor vs two.
For example, full-siblings share both parents as common ancestors, while half-siblings only share one.
We generalize the full- vs half- terminology to distinguish these two types of relatives.
Finally, if a chain consists of both up and down meioses, all up meioses must be crossed before any down meiosis is crossed.
Note the similarity between Wright's path tracing rules discussed earlier and the rules we use here to define a path between two individuals: up and down meioses are analogous to going against and with a directed edge, common ancestors are analogous to double-headed arrows, and matings are analogous to co-paths.

Before we begin writing formulas for the covariance between two individuals, we need to establish an assumption that then leads to a useful approximation:

\begin{assumption}{Single-path}{One path dominates any pair's connection}
The shortest path connecting two individuals in a pedigree is much shorter than any other path connecting them.
\bundles{an outbred pedigree with no loops, the \assumptionid{No-inbreeding} assumption, exclusion of doubled relationships such as two siblings marrying two siblings.}
\end{assumption}
\begin{approximation}{Shortest-path}{Only the shortest path carries covariance}
The covariance between two individuals is taken to be the covariance implied by the shortest path connecting them; every other path is dropped.
\justification{As will be shown later, the covariance implied by a path roughly decays geometrically with the number of steps taken.
Under the \assumptionid{Single-path} assumption, the covariance implied by the shortest path will be much larger than that of all other paths.
At the limit where two distinct paths of equal length exist, such as in doubled relationships, the true covariance can be double that which is implied by the shortest path.}
\end{approximation}

\subsubsection{Covariance implied by matings}
Per our model of assortatvie mating \todo{include ref to earlier assumption box}, matching of individuals acts on the phenotype level such that \todo{Write Corr of m and p equals to rhoy}.
As a consequence, suppose individuals A and D are only connected to each other through the mating of individuals B and C.
Then, by following Wright's path tracing rules, we can decompose the covariance in phenotypes of A and D as follows:
\todo{Write formula that writes Cov of A and D as a function of Cov(A,B), Cov(C,D), mu, etc...}

Furthermore, to obtain the covariances consisting of one or both genetic values, we have to modify our formulas because \todo{write out explanation, include formulas where reasonable}.


\subsubsection{Covariance implied by chains}
\todo{Rewrite parts now that we explained the whole mating covariance stuff just above}

We begin by deriving the covariance between genetic values of two individuals, $A$ and $B$, at the ends of a chain.
...
Note that to obtain this formula for a chain, we actually had to call upon a mating path already.
While this may appear to break our definition of a chain, for clarity, we consolidate the increased covariance that results from assortative mating into a single term to describe the chain connecting one parent to one child.

We can then extend this logic to any pair of \emph{lineal} relatives --- one descended from the other, so that the chain between them runs only upward or only downward.
...
For \emph{collateral} relatives --- neither descended from the other that still share a common ancestor --- we cannot chain the up and down meioses without first asking whether they are \emph{full} relatives, sharing both of the ancestors that join them, or \emph{half} relatives, sharing only one.
This is because full relatives also appear to break the definition of a chain, since there are two distinct paths connecting them, each passing through one of the two common ancestors, who mated with each other.
For clarity, we consolidate those two paths into a single chain carrying their combined covariance.
Under random mating that combination is exactly twice what the corresponding half-relatives carry, and it is this doubling that ties $N_m$ to the degree of relatedness $d$ defined earlier: one extra shared ancestor is worth one degree.

\begin{equation}\label{eq:degree-Nm}
    \boxed{\ d \;=\;
    \begin{cases}
        N_m - 1, & \text{full relatives} \\[2pt]
        N_m,     & \text{half/lineal relatives}
    \end{cases} \ }
\end{equation}
where $N_m$ counts every up and down meiosis needed to connect the pair.
Lineal relatives have no pair of common ancestors to share, so they take $d = N_m$ as well.

We can therefore summarize the genetic covariance between two individuals with a degree of relatedness $d$ to be:
\begin{equation}\label{eq:chain-degree}
    \boxed{\ \Cov[\,g_A,\, g_B\,] \;=\; \left(\frac{1+\rho_g}{2}\right)^{\!d} V_A \ }
\end{equation}
Under random mating this is the textbook $2^{-d}V_A$, which is what fixes the assignment in \eqref{eq:degree-Nm} in the first place.

Equation~\eqref{eq:chain-degree} is exact for lineal relatives and for full collateral relatives, but only approximate for half relatives.
The reason is that a half pair's two \emph{unshared} parents are not unrelated: they are both mates of the shared parent, and so are correlated through a mating that \eqref{eq:chain-degree} does not account for.
This is why the doubling quoted above is exact only under random mating --- assortment inflates half relatives more than full ones, and at $h^2 = \rho_y = \tfrac12$ the ratio is nearer $1.5$ than $2$.
The missing factor is itself a mating term, so we defer it to the next section and pick it up there.
...
When tracing a chain through a pedigree to connect two individuals, there are two types of steps that are performed.
The first is a vertical step, such as going from parent to offspring or vice-versa.
Each vertical step costs one degree of relatedness $d$.
However, note that the actual degree of relatedness $d$ between two individuals depends on all of the distinct chains that connect those two individuals (e.g. full-siblings are connected by both parents).

The second is a horizontal step through a mating.
Analogous to $d$, we define $n$ to be the number of matings that a path crosses.
It is equivalent to the number of co-paths that are crossed under our models' path diagrams.
Recall from Section~\ref{sec:path-analysis} that a chain may cross more than one co-path, provided no two of them belong to the same mating and each costing one mating count $n$.
We focus our derivations on pairs for which $n > 0$ but $d = \infty$,
i.e. pairs who are only connected to each other through matings between themselves or their biological relatives.
Below, we provide examples of such pairs up to $n=3$.

\begin{itemize}
    \item $n=1$: a mated pair, e.g. the parents of full-siblings.
    \item $n=2$: two people who share a mate but are otherwise unrelated, e.g. the non-shared parents of a half-sibling pair.
    \item $n=3$: the two outer parents of a step-sibling pair, one from each child.
\end{itemize}
Figure~\ref{fig:step-sib} shows one pedigree containing all three.
...
\begin{figure}[H]
    \begin{floatcard}
        ...
        \caption{%
            ...
            Every mating count used in this section appears: $n=1$ for the three mated pairs $(p_1,p_2)$, $(p_2,p_3)$ and $(p_3,p_4)$; $n=2$ for $(p_1,p_3)$ and for $(p_2,p_4)$, each of which shares a mate but is otherwise unrelated; and $n=3$ for $(p_1,p_4)$, joined only by the whole chain of three matings.%
        }
        \label{fig:step-sib}
    \end{floatcard}
\end{figure}

Now take two individuals $i$ and $j$ whose only connection is a chain of $n$ matings.
By \eqref{eq:leg-rule}, crossing a co-path $y_a \,\text{---}\, y_b$ breaks the covariance into three factors: how the chain reached $y_a$, the co-path's own coefficient $\mu$, and how it leaves $y_b$.

For $n=1$, a mated pair, there is one co-path and the chain's two ends are the individuals themselves:
$$
    \Cov[\,g_i,\, g_j\,] \;=\; \underbrace{\Cov[\,g_i,\, y_i\,]}_{V_A} \cdot\; \mu \;\cdot \underbrace{\Cov[\,y_j,\, g_j\,]}_{V_A}
    \;=\; \mu\, V_A^2 ,
$$
each end contributing $\Cov[\,g_i,\, g_i + e_i\,] = V_A$ by \assumptionid{GE-indep}.
For $n=2$, where $i$ and $j$ are both mated to some third person $s$, the chain crosses two co-paths and passes \emph{through} $s$.
The middle factor is then $s$'s whole phenotypic variance, because the chain arrives at $y_s$ and leaves from it rather than stopping there:
$$
    \Cov[\,g_i,\, g_j\,] \;=\; V_A \cdot\, \mu \,\cdot \underbrace{\Var[\,y_s\,]}_{V_P} \cdot\, \mu \,\cdot V_A
    \;=\; \mu^2\, V_A^2\, V_P .
$$
Each further mating adds one more crossing and one more person passed through, so in general
\begin{equation}\label{eq:mating-rule}
\begin{gathered}
    \Cov[\,g_i,\, g_j\,] \;=\; \underbrace{\mu^{\,n}}_{\text{crossings}} \cdot \underbrace{V_A^2}_{\text{the two ends}} \cdot \underbrace{V_P^{\,n-1}}_{\text{people passed through}}
    \;=\; \big(\mu V_A\big)\big(\mu V_P\big)^{n-1} V_A \\[6pt]
    \boxed{\ \Cov[\,g_i,\, g_j\,] \;=\; \rho_g\,\rho_y^{\,n-1}\,V_A \ }
\end{gathered}
\end{equation}
where the last step pairs one $\mu$ with one $V_A$ to make $\rho_g$, and each remaining $\mu$ with a $V_P$ to make $\rho_y$, using $\mu V_A = \rho_g$ from \eqref{eq:rhog-t} and $\mu V_P = \rho_y$, which is the same identity with a phenotype in place of a genetic value at one end.
So the \emph{first} mating crossed costs $\rho_g$ and every further one costs $\rho_y$.

Consider next the case where an individual $i$ is connected to some individual $P$ in a pedigree, by vertical and/or horizontal steps, such that the covariance of their genetic values is, $\Cov[\,g_i,\, g_P\,]$.
Write $P'$ for $P$'s mate, and suppose that no chain from $i$ to $P$ crosses \emph{any} of $P$'s matings --- so the only way $i$ reaches $P'$ is by arriving at $P$ and then crossing the one co-path that joins the two of them.
This holds when $i$ is a collateral relative of $P$, or an ancestor of $P$.
It fails when $i$ is one of $P$'s mates, and --- less obviously --- when $i$ is a \emph{descendant} of $P$, because a child of $P$ also carries genes from whichever of $P$'s mates produced it, and those were matched to $P$'s whole phenotype.
Despite their non-biological relatedness, $i$'s genetic value will be more similar to that of $P'$ due to the increased similarity of $P'$ to $P$ as a consequence of assortative mating.

Every chain from $g_i$ to $g_{P'}$ therefore arrives at $y_P$, crosses the co-path, and descends to $g_{P'}$, so \eqref{eq:leg-rule} applies with $P$ and $P'$ as the two mates:
\begin{equation}\label{eq:cross-mating}
\begin{gathered}
\begin{aligned}
    \Cov[\,g_i,\, g_{P'}\,] \;&=\; \Cov[\,g_i,\, y_P\,] \cdot\, \mu \,\cdot \underbrace{\Cov[\,y_{P'},\, g_{P'}\,]}_{V_A} \\[4pt]
    \;&=\; \Big(\Cov[\,g_i,\, g_P\,] + \underbrace{\Cov[\,g_i,\, e_P\,]}_{0}\Big)\cdot \mu\, V_A \\[4pt]
    \;&=\; \Cov[\,g_i,\, g_P\,] \cdot \underbrace{\mu\, V_A}_{\rho_g}
\end{aligned} \\[8pt]
    \boxed{\ \Cov[\,g_i,\, g_{P'}\,] \;=\; \rho_g\,\Cov[\,g_i,\, g_P\,] \ }
\end{gathered}
\end{equation}
The second line splits $y_P$ into its two parts, and the vanishing of $\Cov[\,g_i,\, e_P\,]$ is exactly what the supposition above buys.
Only $P$'s own matings can make $e_P$ correlated with somebody else's genetic value, since a mate of $P$ was chosen partly on $e_P$; excluding those chains is therefore the same as setting that term to zero.
It is worth seeing what the term is worth when it does not vanish.
Take $i$ to be a child of $P$ by a different mate --- a step-child of $P'$, as $o_1$ is of $p_3$ in Figure~\ref{fig:step-sib}.
Then $i$ carries half of that other mate's genetic value, that mate was matched to $y_P$ rather than to $g_P$, and the result is
$$
    \Cov[\,g_{o_1},\, g_{p_3}\,] \;=\; \tfrac12\,\rho_g\big(1 + \rho_y\big) V_A
    \qquad\text{against}\qquad
    \rho_g\Cov[\,g_{o_1},\, g_{p_2}\,] \;=\; \tfrac12\,\rho_g\big(1 + \rho_g\big) V_A ,
$$
so ignoring the condition would substitute $\rho_g$ for $\rho_y$ in one factor and understate the covariance.
That substitution is the same one that separates every class in this section from the plain degree law, and \eqref{eq:lineal} below is the case where it can no longer be avoided.

As a consequence, the covariance of $i$ to $P'$ and $P$'s child, $C$, is modified as follows.
Substituting \eqref{eq:g-transmit} for $g_C$ splits it into the two parents and the segregation residual, and \eqref{eq:cross-mating} supplies the parent that $i$ was \emph{not} traced through:
\begin{equation}\label{eq:one-step}
\begin{gathered}
\begin{aligned}
    \Cov[\,g_i,\, g_C\,] \;&=\; \Cov\!\big[\,g_i,\; \tfrac12 g_P + \tfrac12 g_{P'} + \varepsilon_C\,\big] \\[4pt]
    \;&=\; \tfrac12\Cov[\,g_i,\, g_P\,] \;+\; \tfrac12 \underbrace{\Cov[\,g_i,\, g_{P'}\,]}_{\rho_g\Cov[\,g_i,\, g_P\,]} \;+\; \underbrace{\Cov[\,g_i,\, \varepsilon_C\,]}_{0} \\[4pt]
    \;&=\; \tfrac12\big(1 + \rho_g\big)\Cov[\,g_i,\, g_P\,]
\end{aligned} \\[8pt]
    \boxed{\ \Cov[\,g_i,\, g_C\,] \;=\; \frac{1+\rho_g}{2}\;\Cov[\,g_i,\, g_P\,] \ }
\end{gathered}
\end{equation}
The residual drops out because $\varepsilon_C$ is uncorrelated with both of $C$'s parents and with everything upstream of them.
So one step of descent multiplies a covariance by $\tfrac12(1+\rho_g)$ rather than by $\tfrac12$: half of it is the usual transmission from $P$, and the extra $\rho_g$ is what $C$ inherited from $P'$, who resembles $P$ because they were matched on their phenotypes.
Under random mating $\rho_g = 0$ and \eqref{eq:one-step} is the familiar halving.
...
To properly express the phenotypic covariance between two relatives, we need additional information about their relationship beyond just their degree of relatedness $d$.
First, we use the following terminology to refer to pairs in a pedigree:
Two individuals are \emph{collateral} relatives when neither descends from the other (e.g.\ a sibling pair), and \emph{lineal} when one does (e.g.\ a parent--child pair).
Furthermore, a collateral pair is \emph{full} when they share both of the ancestors that join them, and \emph{half} when they share only one.


\comment{I haven't added to-dos to the following section yet, but feel free to edit it to maintain consistent terminology/symbols}

\subsubsection{Full collateral relatives}

For a full collateral pair, iterating \eqref{eq:one-step} once per step from $\Var[g] = V_A$ gives
\begin{equation}\label{eq:collateral}
\begin{gathered}
    \Cov[\,g_i,\, g_j\,] \;=\; V_A\left(\frac{1+\rho_g}{2}\right)^{\!d} \\[6pt]
    \boxed{\ \Corr[\,y_i,\, y_j\,] \;=\; h^2\left(\frac{1+\rho_g}{2}\right)^{\!d} \ }
\end{gathered}
\end{equation}
The step to the phenotypic correlation is not automatic, and it is what makes this class the clean one: for a collateral pair there is no environmental cross term.
Neither individual's environment had any part in choosing the other's mate, so $\Cov[\,e_i,\, g_j\,] = 0$, and dividing the genetic covariance by $V_P$ is the whole of it.

Equation~\eqref{eq:collateral} is Fisher's result, and it is the form \citet{Eftedal2025PNAS} fit: a correlation of $h^2$ at $d=0$, falling by a constant factor $\tfrac12(1+\rho_g)$ per degree.
Under random mating the factor is $\tfrac12$ and we recover the textbook halving per degree.

\subsubsection{Lineal relatives}
For a lineal pair the chain begins at the ancestor's own phenotype rather than at their genetic value, because the ancestor's environment helped determine who they mated with, and that mate contributed genes to the descendant.
By \eqref{eq:mating-rule} a leg of $V_P$ in place of $V_A$ turns one $\rho_g$ into a $\rho_y$, so exactly one factor of the product changes:
\begin{equation}\label{eq:lineal}
    \boxed{\ \Corr[\,y_i,\, y_j\,] \;=\; h^2\left(\frac{1+\rho_g}{2}\right)^{\!d-1}\left(\frac{1+\rho_y}{2}\right) \ }
\end{equation}
The \emph{genetic} covariance of a lineal pair still obeys \eqref{eq:collateral} exactly; it is only the phenotypic correlation that departs, and it departs in one factor no matter how many generations separate the pair.
At $d=1$ this is the $\tfrac12\VAo(1+\rho_y)$ of Table~\ref{tab:relatives-gen1}, now at equilibrium and at every degree.
Since $\rho_y \geq \rho_g$, a lineal pair is always more similar than a collateral pair of the same degree --- a grandparent resembles a grandchild more than an aunt resembles a niece, though both are second-degree.

\subsubsection{Half relatives}
A half-sibling pair shares one parent, and the two \emph{unshared} parents are both mates of the shared one.
They are therefore separated by two matings, and \eqref{eq:mating-rule} at $n=2$ gives $\Cov = \rho_y\rho_g V_A$ between them rather than zero.
Feeding all four parental covariances into \eqref{eq:g-transmit},
\begin{equation}\label{eq:half}
\begin{gathered}
\begin{aligned}
    \Cov[\,g_i,\, g_j\,] \;&=\; \tfrac14\Big(\underbrace{V_A}_{\text{shared}}
    \;+\; \underbrace{2\rho_g V_A}_{\text{shared with each unshared}}
    \;+\; \underbrace{\rho_y\rho_g V_A}_{\text{the two unshared}}\Big)
\end{aligned} \\[6pt]
    \boxed{\ \Corr[\,y_i,\, y_j\,] \;=\; h^2\left(\frac{1+\rho_g}{2}\right)^{\!d}
    \cdot \underbrace{\frac{1 + 2\rho_g + \rho_y\rho_g}{\big(1+\rho_g\big)^2}}_{\text{half-relative multiplier}} \ }
\end{gathered}
\end{equation}
with $d=2$; half-siblings are collateral, so again the phenotypic correlation is the genetic covariance over $V_P$.
This is the multiplier of \citet{Nagylaki1978}, also derived by path tracing in the supplement to \citet{Sunde2024NatComm}.

The whole of it is the last term.
Replace $\rho_y\rho_g$ by $\rho_g^2$ --- which is what one gets by treating the two unshared parents as separated by two \emph{genetic} legs rather than passing through a phenotype --- and the numerator becomes $(1+\rho_g)^2$ exactly, the multiplier becomes $1$, and \eqref{eq:half} collapses onto \eqref{eq:collateral}.
Since $\rho_y \geq \rho_g$ the multiplier is at least one, so half-relatives are always \emph{more} similar than their degree alone would predict.
It is also the first quantity in this section that cannot be written in $\rho_g$ alone: $\rho_y$ appears separately, and the two are only proportional through $h^2$.

\subsubsection{Relatives-in-law, and the modified degree}
Two people connected only through a marriage --- step-siblings, or the partners of two siblings --- have no common ancestor at all, so $d$ is undefined for them.
Following \citet{Eftedal2025PNAS} we index them instead by a \emph{modified degree} $\tilde{d}$: the number of transmission steps from one individual to their end of the connecting mated pair, plus the steps from the other individual to the other end.
Step-siblings have $\tilde{d} = 2$, one step each.
For every pair that does have a common ancestor, $\tilde{d} = d$, so $\tilde{d}$ extends the index rather than replacing it, and we use it from here on in any formula meant to cover both.

The chain now crosses the connecting mating once, contributing $\rho_g$, and the step on each side that is \emph{adjacent} to that mating has a phenotype leg, contributing $\tfrac12(1+\rho_y)$ rather than $\tfrac12(1+\rho_g)$.
Every remaining step is an ordinary transmission:
\begin{equation}\label{eq:inlaw}
    \boxed{\ \Corr[\,y_i,\, y_j\,] \;=\; h^2\,\rho_g\left(\frac{1+\rho_y}{2}\right)^{\!2}\left(\frac{1+\rho_g}{2}\right)^{\!\tilde{d}-2} \ }
\end{equation}
The two squared factors are one per side, so \eqref{eq:inlaw} needs $\tilde{d} \geq 2$; an in-law is at least one step from the couple that connects them.
The leading $\rho_g$ is why in-laws resemble each other at all: they are connected by a single mating, and nothing else.

Two things are worth recording about \eqref{eq:inlaw}.
The first is that $\tilde{d}$ really is sufficient.
A pair one step and three steps from the connecting couple has the same expected correlation as a pair two steps and two steps from it, which is what licenses indexing in-laws by a single number rather than by the shape of the connection.
The second is that \eqref{eq:inlaw} disagrees with the expression fitted by \citet{Eftedal2025PNAS}, who take $h^2\rho_g\big(\tfrac12(1+\rho_g)\big)^{\tilde{d}}$ --- the ordinary rate factor, raised to the whole modified degree.
The difference is the two adjacent steps, and it makes \eqref{eq:inlaw} the larger of the two: at $h^2 = \tfrac12$ and $\rho_y = \tfrac12$ a step-sibling correlation of $0.070$ against their $0.049$.
It is at least suggestive that they report their own fitted model \emph{under}-predicting the in-law classes --- step-siblings by about a factor of two --- which is the direction this correction points.

\subsubsection{When degree is enough}
Collecting the four cases, Table~\ref{tab:degree-classes} states what each class costs relative to the plain degree law.
...
\begin{table}[H]
    ...
    \footnotesize
    \begin{tabular}{lll}
        class & $\Corr[\,y_i,\, y_j\,]$ & needs \\
        \hline
        collateral, full
            & $h^2\left(\dfrac{1+\rho_g}{2}\right)^{\tilde{d}}$
            & $\tilde{d}$ and $\rho_g$ \\
        lineal
            & $h^2\left(\dfrac{1+\rho_g}{2}\right)^{\tilde{d}-1}\left(\dfrac{1+\rho_y}{2}\right)$
            & $\tilde{d}$, $\rho_g$ and $\rho_y$ \\
        collateral, half
            & $h^2\left(\dfrac{1+\rho_g}{2}\right)^{\tilde{d}} \dfrac{1+2\rho_g+\rho_y\rho_g}{(1+\rho_g)^2}$
            & $\tilde{d}$, $\rho_g$, $\rho_y$, and that the pair is half \\
        in-law
            & $h^2\,\rho_g\left(\dfrac{1+\rho_y}{2}\right)^{2}\left(\dfrac{1+\rho_g}{2}\right)^{\tilde{d}-2}$
            & the same, plus which couple connects them \\
    \end{tabular}
    \caption{%
        Expected phenotypic correlation by class, at equilibrium, for a pair whose only connection is the one named.
        Every row is a product of per-step factors, and the classes differ only in how many of those factors carry $\rho_y$ in place of $\rho_g$: none for a full collateral pair, one for a lineal pair, two for an in-law pair, and a fractional correction for a half pair.
        Under random mating $\rho_g = \rho_y = 0$, every row collapses to $h^2 2^{-\tilde{d}}$ except the in-law row, which collapses to zero.%
    }
    \label{tab:degree-classes}
    \end{floatcard}
\end{table}

So the degree is enough, but only within a class.
Two individuals' expected covariance is fixed by $\tilde{d}$ together with three facts that $\tilde{d}$ does not carry: whether one descends from the other, whether their connection runs through one shared ancestor or two, and whether it runs through a marriage.
Assortment is what makes those distinctions matter.
Set $\rho_y = 0$ and every row of Table~\ref{tab:degree-classes} that has a common ancestor becomes $h^2 2^{-\tilde{d}}$: degree alone suffices, and the classes are indistinguishable.
