...
We begin by noticing that the $c$ term we introduced earlier simplifies substantially and makes the recursion more manageable:

We rewrite the formula for $c$ from \eqref{eq:c-def} as two separate sums, each of which will be simplified:
\begin{equation}\label{eq:c-split}
    c^{(t+1)}_k \;=\; \sum_{l=1}^{M} \beta_l\Big(s^{(t+1)}_{kl} + a^{(t+1)}_{kl}\Big)
    \;=\; \underbrace{\sum_{l} \beta_l\,a^{(t+1)}_{kl}}_{\text{(i)}}
    \;+\; \underbrace{\sum_{l} \beta_l\,s^{(t+1)}_{kl}}_{\text{(ii)}} .
\end{equation}

\paragraph{(i) The across-gamete half.}
The across-gamete part of the equation can itself be written in terms of $c$ terms from the previous generation, first described in \eqref{eq:a-recursion}.
After pulling out common terms, we are left with a summation that can be simplified according to \eqref{eq:VA-from-as}.
We also plug in the expression for $\mut$:
\begin{equation}\label{eq:c-part-a}
    ...
    \;=\; \frac{\rho_y\,\VAt}{2\,\VPt}\, c^{(t)}_k ,
\end{equation}
The sum over $M$ variants that \eqref{eq:a-recursion} carries has now become a single ratio of variance components.

\paragraph{(ii) The same-gamete half.}
We can fill in the recursive expression for $s^{(t+1)}_{kl}$ according to \eqref{eq:s-recursion}, which lets us write it in terms of $c^{(t)}_k$ according to \eqref{eq:c-def}.
...
\begin{aligned}
    ...
    &\;=\; \underbrace{\tfrac12 \sum_{l} \beta_l\big(s^{(t)}_{kl} + a^{(t)}_{kl}\big)}_{c^{(t)}_k/2}
...
\paragraph{The collapsed recursion.}
Adding \eqref{eq:c-part-a} and \eqref{eq:c-part-s} gives
\begin{equation}\label{eq:c-recursion}
    \boxed{\ c^{(t+1)}_k \;=\; \underbrace{\tfrac12\left(1 + \frac{\rho_y\,\VAt}{\VPt}\right) c^{(t)}_k}_{\text{inherited}}
...
We've previously thought of inherited alleles as having a directly-inherited component from the parent (equivalent to half the parent's genotype) plus segregation deviation that is uncorrelated with the parent's genotype.
Both contribute covariance with the offspring's phenotype.
...
This term is not linearly recursive, since $a^{(t)}_{kk} = \mu^{(t-1)}\big(c^{(t-1)}_k\big)^2$ by \eqref{eq:a-recursion}.

The recursion is now more manageable because equation \eqref{eq:c-recursion} only tracks $M$ numbers, and the variants are coupled \emph{only} through the single ratio $\VAt/\VPt$ --- which is itself recoverable from the $c^{(t)}_k$ by \eqref{eq:VA-from-as}.
The state of the population, as far as the dynamics are concerned, is the vector $c^{(t)}$.
