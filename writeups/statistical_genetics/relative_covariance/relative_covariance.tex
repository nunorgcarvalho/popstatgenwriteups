...
We write the aggregate residual as $\varepsilon_o$ rather than $\epsilon_o$: unlike the per-variant deviations $\epsilon^{(u)}_{ok}$ it is built from, it is weighted by the effect sizes, so it is measured on the scale of a genetic value rather than of an allele.
...
Because $\varepsilon_o$ is uncorrelated with both parents and across siblings, \eqref{eq:g-transmit} is a linear model of exactly the kind Section~2.2.1 draws: two directed edges of $\tfrac12$ into the offspring's genetic value, and a disturbance carrying \eqref{eq:g-seg-var}.
