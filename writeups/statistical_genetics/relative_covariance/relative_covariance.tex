...
It is the covariance between two \emph{different} causal variants in two \emph{different} people --- a quantity that is zero under random mating, and for which there is no common ancestor to appeal to.
Multiplying the coefficients along it gives $\Cov[x_{m,1}, x_{p,2}] = \beta_1\beta_2\rho_y/\VPo$ by \eqref{eq:copath-mu}.
...
We also generalize from two variants back to the genome-wide sum of \assumptionid{Additivity}, $g_i = \sum_{k=1}^{M} \beta_k x_{ik}$, for which $\VAo = \sum_k \beta_k^2$ and $\VPo = \VAo + V_E$.
No separate count of causal variants is needed: a non-causal variant carries $\beta_k = 0$ and drops out of every sum below, so summing over all $M$ costs nothing.
...
\begin{itemize}
    \item $\Cov[\,x_{mk},\, x_{pl}\,] = \beta_k\beta_l\,\rho_y/\VPo \neq 0$, for \emph{every} pair of variants $k$ and $l$ --- the same variant in the two mates ($k=l$) included, and vanishing only where one of the two is non-causal.
