...
These derivations are based on Supplementary Note 1 of Yang et al. \citep{yang2014natgen}. They describe the model behind GCTA's implementation of mixed linear model (MLM) associations for determining SNP effects in a dataset containing cryptic relatedness.
...
Where:
\begin{itemize}
    \item $\by \in \R^N$: outcomes.
    \begin{itemize}
        \item $N$: number of samples.
    \end{itemize}
    \item $\bK \in \R^{N \times (K+1)}$: design matrix of covariates and intercept placeholder term.
    \begin{itemize}
        \item $K$: number of covariates.
    \end{itemize}
    \item $\bc \in \R^{K+1}$: covariate coefficients, including intercept term.
    \item $\bg \in \R^N$: genetic effects.
    \item $\bepsilon \in \R^N$: non-genetic effects (i.e. error or noise term).
\end{itemize}
...
$$ \bg \sim \mathcal{N} (\bzero, \bA \varg)$$
Where $\bA \in \R^{N \times N}$ has elements denoting the genetic relatedness between individuals $j$ and $k$ as:
...
Where $\bw_i \in \R^N$ is a vector of mean-adjusted genotypes, $w_{ij} = x_{ij} - 2 p_i$, and $b_i$ is its fixed effect we are trying to estimate. We can combine our fixed effects as follows:
\begin{equation*}
