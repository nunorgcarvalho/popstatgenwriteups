...
Largely adapted from the respective Wikipedia article \citep{wikipedia2026webGLS}.
...
\begin{itemize}
    \item $\by \in \R^N$: outcome vector.
    \begin{itemize}
        \item $N$: number of samples
    \end{itemize}
    \item $\bX \in \R^{N \times K}$: design matrix.
    \begin{itemize}
        \item $K$: number of predictors
    \end{itemize}
    \item $\bbeta \in \R^K$: coefficients vector.
    \item $\bepsilon \in \R^N$: error term.
\end{itemize}
...
We also assume that the variance of the error term given $\bX$ is described by an invertible covariance matrix, $\bOmega \in \R^{N \times N}$:
$$ \Cov[ \bepsilon | \bX ] = \bOmega $$

We further assume that the error term follows a multivariate normal distribution with mean $\bzero$ and covariance $\bOmega$:
$$ \bepsilon \sim \N (\bzero, \bOmega) $$
...
\begin{align*}
    \bzero &= \frac{\partial}{\partial \bb} [ \by^{\T} \bOmega^{-1} \by + (\bX \bb)^{\T} \bOmega^{-1} (\bX \bb) - 2(\bX \bb)^{\T} \bOmega^{-1} \by ]\\
...
Note that in Ordinary Least Squares (OLS), the covariance matrix is an identity matrix, $\bOmega = \bI$. That is, the residuals are uncorrelated with each other. This simplifies the above equation to:
...
\begin{align*}
    ...
    &= \bzero + \E[ \Var[\bepsilon | \bX] ] + \Var[ \E[\bepsilon | \bX]] \\
    &= \E[ \bOmega ] + \Var[ \bzero ] \\ 
    &= \bOmega
\end{align*}

Let's define $\bA \in \R^{K \times N}$ as $\bA = (\bX^{\T} \bOmega^{-1} \bX)^{-1} \bX^{\T} \bOmega^{-1}$, such that $\hat{\bbeta} = \bA \by$. Furthermore, note that since $\bOmega$ is a covariance matrix, it (and its inverse) is symmetric: $\bOmega = \bOmega^{\T}$. The same symmetry property applies to $\bX^{\T} \bOmega^{-1} \bX$. We can then solve:
