...
Large portions are adapted from Wikipedia articles relating to the relevant topics \citep{wikipedia2026webCovMatrix,wikipedia2026webCorrelation,wikipedia2026webRandomEffects,wikipedia2026webMVN,wikipedia2026webCholesky}.
...
The covariance matrix of such a vector, $\bK_X \in \R^{N \times N}$, has a $(i,j)\textsuperscript{th}$ element that represents the covariance between $X_i$ and $X_j$:
$$ K_{X,ij} = \Cov[X_i,X_j] = \E[(X_i - \E[X_i]) (X_j - \E[X_j])]$$
Diagonal elements of $\bK_X$, where $i=j$, describe the variance of the scalar random variable $X_i$.
...
Note that $K_{X,ij} = R_{X,ij} \sigma_{X_i} \sigma_{X_j}$. We can write this in terms of their respective matrices by defining $\bsigma_X \in \R^{N \times N}$ as the diagonal matrix with random variable standard deviations on the diagonal, $\bsigma_X = (\diag[\bK_X])^{-\frac12} =  (\bone (\sigma_{X_1}, \sigma_{X_2}, ..., \sigma_{X_N})^{\T}) \circ  \bI_N$, allowing us to write:
...
$$ \bx^{\T} \bK \bx \geq 0 \text{ for all } \bx \in \R^N $$
...
From now on, we will assume that $\bX$ consists of repeated draws of the same random variable, $X$, meaning that all diagonal entries of $\bK_X$ are the same and equal to the expected variance of $\bX$:
$$ \bX \sim \N (\bzero,\sigma_X^2 \bR_X) $$
...
$$ \bepsilon \sim \N (\bzero, \sigma_\epsilon^2 \bI_N)$$
...
We can define a design matrix $\bZ_X \in \R^{N \times N_X}$ that maps the $N$ samples to $N_X$ clusters. In our example,
...
Defining such a matrix allows us to define a smaller correlation matrix for the clusters, $\bA_X \in \R^{N_X \times N_X}$, capturing the physical proximity between cities in our example.
We'll continue to assume that the random effects coming from each cluster have the same variance, $\sigma_X$.

We can define our sample-level covariance matrix under this new parameterization as $\bK_X = \sigma_X^2 \bZ_X \bA_X \bZ_X^{\T}$, where $\bK_X \in \R^{N \times N}$, also allowing us to redefine the distribution of the random effect itself:
$$ \bX \sim \N (\bzero, \sigma_X^2 \bZ_X \bA_X \bZ_X^{\T} ) $$
If your sample consists of no discrete clusters (or you want to think of each sample as forming its own cluster), then $\bZ_X \in \R^{N \times N}$ is simply an identity matrix, and $\bA_X \in \R^{N \times N}$ is equivalent to the $\bR_X$ matrix we defined earlier.
